...
class AfterEffectsSceneCollector(HookBaseClass):
    ...
    def process_current_session(self, settings, parent_item):
        logger.info('===================== Collector - Process Current Session =====================')
        logger.info(str(settings))
        logger.info(str(parent_item))
        ...
        tmp = fields_from_work_template['version']
        ver = 'v' + f'{tmp:03}'      

        composition_name = ['motion', ver]
        
        logger.info('****************** print ******************')
        logger.info(str(parent_item))
        logger.info(str(work_template))
        logger.info('****************** print ******************')
        ...
        for i, queue_item in enumerate(self.parent.engine.iter_collection(adobe.app.project.renderQueue.items)):
            ...
            # if any(name in queue_item.comp.name for name in composition_name):
            if all(name in queue_item.comp.name for name in composition_name):
                queue_item = queue_item

                logger.info('****************** QUEUE ITEM ******************')
                logger.info(str(queue_item))
                logger.info(str(queue_item.comp.name))
                ...
                comp_item_name = "Render Queue Item #{} - {} - {}".format(i + 1, queue_item.comp.name, action)

                self.__create_comp_publish_item(parent_item, comp_item_name, comment, queue_item, render_paths, i, work_template)
                
                logger.info('****************** ITEM INFO ******************')
                logger.info(str(parent_item))
                logger.info(str(comp_item_name))
    ...
    def __get_work_template_for_item(self, settings):
        logger.info('===================== Collector - Get Work Template For Item =====================')
        logger.info(str(settings))

        # try to get the work-template
        work_template_setting = settings.get("Work Template")
        logger.info('****************** Work Template ******************')
        logger.info(str(work_template_setting))

        if work_template_setting:
            logger.info('****************** RETURN Work Template Setting ******************')    
            logger.info(str(self.parent.engine.get_template_by_name(work_template_setting.value)))
            logger.info('****************** RETURN Work Template Setting ******************')    
    ...
    def __get_project_publish_item(self, settings, parent_item):
        logger.info('===================== Collector - Get Project Publish Item =====================')
        logger.info(str(settings))
        logger.info(str(parent_item))
        """
        ...
        logger.info('****************** Work Template ******************')
        logger.info(str(work_template))
        logger.info('****************** RETURN Project Item ******************')    
        logger.info(str(project_item))
        logger.info('****************** RETURN Project Item ******************')
    ...
    def __create_comp_publish_item(self, parent_item, name, comment, queue_item, render_paths, queue_index, work_template=None):
        logger.info('===================== Collector - Create Comp Publish Item =====================')
        logger.info(str(parent_item))
        logger.info(str(name))
        logger.info(str(comment))
        logger.info(str(queue_item))
        logger.info(str(render_paths))
        logger.info(str(queue_index))
        logger.info(str(work_template))
        ...
        logger.info('****************** COMP ITEM ******************')
        logger.info(str(comp_item))
        logger.info('****************** COMP ITEM ******************')
