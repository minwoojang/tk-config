...
class AfterEffectsCopyRenderPlugin(HookBaseClass):
    ...
    def accept(self, settings, item):
        logger.info('===================== Copy Rendering - Accept =====================')
        """
    ...
    def validate(self, settings, item):
        logger.info('===================== Copy Rendering - Validate =====================')
        """
    ...
    def publish(self, settings, item):
        logger.info('===================== Copy Rendering - Publish =====================')
        """
        ...
        for r in render_paths:
            if re.search(r'\.\d{4}\.', r) or '####' in r or '%04d' in r:
                ...
                render_mov_path_template_str = 'aftereffects_asset_render_pub_mov'
            else:
                ext = r.split('.')[-1]
                tmp = [t for t in render_mov_path_template_str if ext in t]
                render_seq_path_template_str = 'aftereffects_asset_render_pub_tif'
                render_mov_path_template_str = ', '.join(tmp)
        ...
        new_renderpaths = []
        for each_path in self.__iter_publishable_paths(queue_item, queue_item_index, render_paths, work_template, render_mov_path_template, render_seq_path_template):
            new_renderpaths.append(each_path)

        item.properties["renderpaths"] = new_renderpaths

        logger.info('****************** Publish DATA ******************')
        logger.info(str(queue_item))
        logger.info(str(new_renderpaths))
        logger.info(str(render_seq_path_template_str))
        logger.info(str(render_seq_path_template))
        logger.info(str(render_mov_path_template_str))
        logger.info(str(render_mov_path_template))
        logger.info('****************** Publish DATA ******************')

    def __is_acceptable(self, settings, item):
        logger.info('===================== Copy Rendering - IS Acceptable =====================')
        """
        ...
        logger.info("****************** Check output module configuration ******************")
        om_state = self.__output_modules_acceptable(item, queue_item, default_mov_output_module, default_seq_output_module, check_output_module, force_output_module)
        ...
        #     logger.info(str(render_mov_path_template.keys["extension"]))
            
        #     render_mov_path_template.keys["extension"].default = ("avi" if sys.platform == "win32" else "mov")
        #     logger.info(str(render_mov_path_template.keys["extension"].default))

        # check template configuration
        logger.info("****************** Check template configuration ******************")
        t_state = self.__templates_acceptable(work_template, render_seq_path_template, render_mov_path_template, project_path)
        ...
        logger.info("****************** Check if the templates are matching ******************")
        ext_state = self.__template_extension_match_render_paths(render_paths, render_seq_path_template, render_mov_path_template)

        if ext_state != self.FULLY_ACCEPTED:
            return ext_state

        logger.info('****************** Acceptable DATA ******************')
        logger.info(str(queue_item))
        logger.info(str(render_paths))
        logger.info(str(default_seq_output_module))
        logger.info(str(default_mov_output_module))
        logger.info(str(render_seq_path_template_str))
        logger.info(str(render_seq_path_template))
        logger.info(str(render_mov_path_template_str))
        logger.info(str(render_mov_path_template))
        logger.info(str(check_output_module))
        logger.info(str(force_output_module))
        logger.info('****************** Acceptable DATA ******************')

        return self.FULLY_ACCEPTED

    def __template_extension_match_render_paths(self, render_paths, seq_template, mov_template):
        logger.info('===================== Copy Rendering - TEMPLATE Extension match render path =====================')
        """
        ...
        for each_path in render_paths:
            ...
            if ("extension" in path_template.keys and path_template.keys["extension"].default):
                template_ext = re.sub(r"^[\.]*", ".", path_template.keys["extension"].default)
    ...
    def __templates_acceptable(self, work_template, seq_template, mov_template, project_path):
        logger.info('===================== Copy Rendering - TEMPLATE Acceptable =====================')
        """
    ...
    def __output_modules_acceptable(self, publish_item, queue_item, mov_template, seq_template, check=True, force=True):
        logger.info('===================== Copy Rendering - OUTPUT Modules acceptable =====================')
        """
        ...
        for i, output_module in enumerate(self.parent.engine.iter_collection(queue_item.outputModules)):
            ...
            if not i:
                ...
                logger.info(str(output_module_names))
                msg = (
            ...
            template_name = output_module.name
            ...
            logger.info('****************** OUTPUT Module & Template Name******************')
            logger.info(str(queue_item.outputModules))
            logger.info(str(output_module))
            logger.info(str(template_name))

            def fix_output_module(output_module=output_module, template=template_name, queue_item=queue_item, item=publish_item, engine=self.parent.engine):
                logger.info('===================== Copy Rendering - FIX OUTPUT Module =====================')                
                """
    ...
    def __iter_publishable_paths(self, queue_item, queue_item_idx, render_paths, work_template, mov_template, seq_template):
        logger.info('===================== Copy Rendering - Iter Publishable Paths =====================')
        """
    ...
    def __get_save_as_action(self):
        logger.info('===================== Copy Rendering - GET SAVE AS action =====================')
        """
