...
class MayaSessionCollector(HookBaseClass):
    ...
    def _collect_fbx_caches(self, parent_item):
        # component_name = parent_item.context.entity['name']
        fbx_item = parent_item.create_item(
            "maya.session.component.FBX",
            "FBX ",
            "Export FBX"
        )

        self.logger.info("FBX Item created: %s" % fbx_item)
    ...
    def collect_component(self,parent_item):
        ...
        if pipeline_step in ['Model',]:
            obj_item = parent_item.create_item(
            "maya.session.component.obj",
            "Obj",
            "Export Obj for lookdev"
            )

            obj_icon_path = os.path.join(
                self.disk_location,
                os.pardir,
                "icons",
                "obj.png"
            ...
            obj_item.properties['name'] = component_name
            obj_item.set_icon_from_path(obj_icon_path)
